...
이 강의에서는 아래와 같은 **밸런스 게임**을 직접 만들어봅니다.

![밸런스 게임 데모](/images/balance-game-demo.png)
...
강의에서 함께 코드를 작성하고, 후반에 학생들이 직접 **댓글 기능(새로운 Entity)을 추가하는 실습**을 진행합니다.

## 서비스 구성

이 밸런스 게임을 만들기 위해서는 **프론트엔드 → 백엔드 → 데이터베이스** 구조가 필요합니다.
...
2. **서비스 구성 개요** - 전체 아키텍처 설명
3. **백엔드 개발** - Spring Boot REST API
4. **프론트엔드 개발** - React 컴포넌트 + API 연동
