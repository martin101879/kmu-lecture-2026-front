...
> 템플릿 파일(`cloudformation.yaml`)은 강의 당일 제공됩니다. 파라미터로 DB 비밀번호를 입력해야 하며, 나머지 설정은 기본값을 사용합니다.

4. **스택 이름** 입력: `balance-game` — 각자 별도의 AWS 계정에서 생성하므로 동일한 이름을 사용합니다
...
|--------|------|
| ECR | Docker 이미지 저장소 |
| ECS Cluster + Service | 컨테이너 실행 (Express Mode) |
| RDS (MySQL) | 데이터베이스 |
