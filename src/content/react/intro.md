...
> 엄밀히 React는 **프레임워크가 아닌 라이브러리**입니다. UI 렌더링에 집중하고, 라우팅/상태관리 등은 별도 라이브러리를 조합해서 사용합니다. 다만 실무에서는 React + 주변 라이브러리를 묶어 "React 프레임워크"로 부르는 경우가 많고, 앞서 배운 Convention over Configuration 등 프레임워크적 특성도 갖고 있습니다.
...
| 프론트엔드 기술 | 개발사 | 특징 |
|---------------|--------|------|
| **React** | Meta (Facebook) | 가장 큰 생태계, 채용 시장에서 가장 많이 요구 |
| Vue | Evan You (개인) | 학습이 쉬움, 국내에서도 많이 사용 |
| Angular | Google | 대규모 엔터프라이즈에서 사용, 러닝 커브 높음 |
| Svelte | Rich Harris (Vercel) | 컴파일 기반, 빠르지만 생태계가 작음 |
...
SPA:
  브라우저 → 처음 한 번만 HTML/JS 로드 → 이후 데이터가 필요하면 API 호출 → 화면 부분 업데이트
```

SPA는 처음 로딩 시 React 애플리케이션 전체를 다운로드하고, 이후에는 페이지 전환 없이 화면을 동적으로 업데이트합니다. 새 데이터가 필요한 경우에만 백엔드 API를 호출하여 JSON 데이터를 주고받습니다. 이 강의에서 만드는 밸런스 게임이 바로 이 방식입니다.

### CSR vs SSR

SPA와는 별개로, **렌더링을 어디서 하느냐**에 따라 CSR과 SSR로 나뉩니다. 여기서 렌더링이란 **HTML 문서를 만드는 것**입니다. 즉, 브라우저가 만드느냐(CSR) 서버가 만드느냐(SSR)의 차이입니다.
...
React는 실제 DOM이 아닌 **가상 DOM** (Virtual DOM)을 메모리에 유지하고, state가 변경되면 가상 DOM을 먼저 업데이트한 뒤 실제 DOM과의 차이만 반영합니다. 가상 DOM은 특별한 기술이 아니라, UI 구조를 **JavaScript 객체 트리**로 표현한 것입니다. 브라우저 DOM을 직접 조작하는 대신 가벼운 JS 객체끼리 비교하므로 빠릅니다.
...
| 렌더링 대상 | 도구 | 설명 |
|-----------|------|------|
| 웹 브라우저 (DOM) | [React DOM](https://react.dev) | 웹 애플리케이션 개발 (이 강의) |
| iOS / Android | [React Native](https://reactnative.dev) | 모바일 앱 개발 |
| 터미널 | [Ink](https://github.com/vadimdemedes/ink) | 커맨드 라인 프로그램 개발 |
