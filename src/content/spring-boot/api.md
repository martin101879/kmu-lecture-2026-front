...
![Docker 계층 구조](/images/docker-host-container.svg)

내 PC(호스트) 위에 Docker Engine이 동작하고, 그 위에서 각각 격리된 컨테이너들이 실행됩니다. 컨테이너는 서로 독립적이며, 내 PC에 직접 소프트웨어를 설치하지 않아도 됩니다.
...
```yaml
name: balance-api                      # docker compose 프로젝트 이름

services:                              # 실행할 컨테이너 목록
  mysql:                               # 서비스 이름 (여러 서비스를 정의할 수 있음)
    image: mysql:8.0                   # 사용할 Docker 이미지 (Docker Hub에서 다운로드)
    container_name: mysql              # 컨테이너 이름 (docker ps에서 보이는 이름)
    environment:                       # 컨테이너 내부 환경 변수 (MySQL 초기 설정)
    ...
    ports:
      - "3306:3306"                    # 호스트 포트:컨테이너 포트 매핑
                                       # → localhost:3306으로 접속하면 컨테이너의 3306으로 전달
    volumes:
      - ./mysql-data:/var/lib/mysql    # 호스트 경로:컨테이너 경로
                                       # → 컨테이너를 삭제해도 데이터가 호스트에 유지됨
```
...
`localhost:3306`으로 접속하면 Docker 컨테이너 안의 MySQL에 연결됩니다. docker-compose.yml에서 `ports: "3306:3306"`으로 호스트 포트와 컨테이너 포트를 매핑했기 때문입니다.
